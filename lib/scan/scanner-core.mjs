...
// Lazy-loaded DB reference — set by callers that want dual-write to SQLite
let _db = null;
...
/**
 * Resolve one or more region keys to an array of ISO country codes.
 * Returns all country_codes from matching regions, deduplicated.
 * If a region has no country_codes (e.g. "global"), it returns [] — the
 * caller should interpret that as "search everything".
 * @param {string|string[]} regionKeys - e.g. "uae" or ["north-america", "europe"]
 * @returns {{ codes: string[], missing: string[], regionLabels: object }}
 */
export function resolveRegionCountries(regionKeys) {
    const regions = loadRegions();
    const keys = Array.isArray(regionKeys) ? regionKeys : [regionKeys];
    const codes = new Set();
    const missing = [];
    const regionLabels = {};
    for (const key of keys) {
        const clean = key.trim().toLowerCase().replace(/\s+/g, '-');
        const region = regions[clean];
        if (!region) {
            missing.push(key);
            continue;
        }
        regionLabels[clean] = region.label;
        if (region.country_codes) {
            for (const c of region.country_codes) codes.add(c);
        }
    }
    return { codes: [...codes], missing, regionLabels };
}
...
/**
 * Set the database reference for dual-write support.
 * Call once before using appendToPipeline if SQLite persistence is desired.
 * @param {object} db - Database module with addPipelineItems, addScanHistory
 */
 ...
 * Also writes to SQLite if database reference was set via setDb().
...
export function appendToPipeline(jobs, tag) {
    ...
    // Dual-write to SQLite if DB reference is set
    if (_db && typeof _db.addPipelineItems === 'function') {
        const items = jobs.map(j => ({
            url: j.url,
            company: j.company,
            role: j.title,
            tag: tag ? tag.replace(/[[\]]/g, '') : '',
            source: j.portal || 'scanner',
        }));
        _db.addPipelineItems(items);
    }
}

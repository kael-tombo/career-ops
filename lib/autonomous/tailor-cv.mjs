 ...
 *
 * Reads base CV (cv.md) + job description, uses LLM (Gemini) to
 * generate a tailored CV, converts to PDF for application submission.
 *
 ...
 * @param {object} job - { url, company, role, description }
 * @param {object} [options]
 * @returns {Promise<{tailoredPath: string|null, success: boolean, error?: string}>}
 */
export async function tailorCV(job, options = {}) {
  const { company, role, url } = job;
  const description = job.description || job.jd || '';

  if (!existsSync(CV_PATH)) {
    return { tailoredPath: null, success: false, error: 'No cv.md found' };
  }

  if (!geminiAvailable) {
    return { tailoredPath: null, success: false, error: 'Gemini not available' };
  }
  ...
  return { tailoredPath: outputPath, success: true };
}

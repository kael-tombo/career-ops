 ...
 * description, uses LLM (multi-provider gateway) to generate a tailored
 * CV, converts to PDF for application submission.
 *
...
export const TAILOR_DEFAULTS = {
  model: process.env.LLM_CV_MODEL || 'gpt-4o-mini',
  temperature: 0.3,
  maxTokens: 4096,
};
...
async function callLLM(prompt, options = {}) {
  const { askGemini } = await import('../ai/gemini.mjs');
  return askGemini(prompt, {
    temperature: options.temperature ?? TAILOR_DEFAULTS.temperature,
    maxTokens: options.maxTokens || TAILOR_DEFAULTS.maxTokens,
    timeoutMs: options.timeoutMs ?? 120000,
    models: [options.model || TAILOR_DEFAULTS.model],
  });
}

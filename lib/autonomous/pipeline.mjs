...
const EVAL_MIN_SCORE = parseFloat(process.env.MIN_SCORE || '3.5');
...
export async function stageScan(options = {}) {
  ...
  try {
    const { loadPortalsConfig, getSeenUrls } = await import('../../scan-portals.mjs');
  ...
  try {
    ...
    const orch = new GlobalOrchestrator({
      queries: options.queries || ['software engineer', 'product manager', 'data scientist'],
      maxTotal: options.maxGlobal || 200,
      concurrency: 2,
      rpm: 20,
    });
    const { jobs } = await orch.runFullSweep();
    for (const job of jobs) {
      addToPipeline(job.url || job.link, job.company || '', job.title || '', 'global-sweep');
    }
    results.globalJobs = jobs.length;
    results.totalNew += jobs.length;
    await orch.close();
  } catch (err) {
...
export function addToPipeline(url, company, role, source) {
  const entry = `- [ ] ${url} | ${company} | ${role} | ${source}\n`;
...
export async function stageEvaluate(options = {}) {
  ...
  const batch = pending.filter(p => !p.checked && !p.evaluated).slice(0, 20);
  ...
  for (const item of batch) {
    try {
      ...
      let score = await evaluateJD(jd, item.url);
      ...
      markPipelineEvaluated(item.url);
      results.push({ url: item.url, score, status: score >= EVAL_MIN_SCORE ? 'eligible' : 'below-threshold' });
      evaluated++;
...
async function evaluateJD(jd, url) {
  if (!jd) return 2.0;

  const apiKey = process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY;
  if (!apiKey) return 3.0;
  ...
  const prompt = `Evaluate this job for ${profile.name || 'the candidate'}.

## Candidate Profile
${JSON.stringify(profile, null, 2)}

${memoryBriefing ? `## AI Memory (learned from past evaluations)
...
Respond with ONLY a JSON object: {"score": 3.5, "reasoning": "..."}`;
  ...
  if (jsonMatch) {
    try {
      ...
      return Math.min(5, Math.max(1, parsed.score));
    } catch {
      return 3.0;
    }
  }
...
function loadProfileForEval() {
  ...
  try {
    ...
    const profile = load(readFileSync(yamlPath, 'utf-8'));
    return {
      name: profile.candidate?.full_name || 'Candidate',
...
function markPipelineEvaluated(url) {
  ...
  for (const job of eligible) {
    ...
    try {
      if (autoTailor) {
        ...
        const tailorResult = await tailorCV({ company: job.company, role: job.role, url: job.url, description: desc });
...
function updateTracker(job, result) {
  ...
  const entry = `\n| ${num} | ${today} | ${job.company || 'Unknown'} | ${job.role || 'Unknown'} | ${result.status === 'submitted' ? 'Applied' : 'Evaluated'} | ${job.score ? `${job.score}/5` : 'N/A'} | ✅ | [${num}](reports/${String(num).padStart(3, '0')}-${slug}-${today}.md) | Auto-applied via pipeline |`;
...
export async function runFullPipeline(options = {}) {
  ...
  const evalResult = await stageEvaluate(options);

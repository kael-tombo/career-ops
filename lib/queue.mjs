...
export class JobQueue {
  constructor(options = {}) {
    ...
    this._handlers = {
      ...
      evaluate: async (payload) => {
        ...
        const { tryFetch } = await import('./fetch-jd.mjs');
      ...
      'mass-scan': async () => {
        const { runMassScan } = await import('./discovery/mass-scan.mjs');
      ...
      'global-sweep': async (payload) => {
        ...
        const { getCountryCodes } = await import('./global/search-engines.mjs');
      ...
      'mass-apply': async (payload) => {
        const { runMassApply } = await import('./auto-apply/mass-apply.mjs');
      ...
      'pipeline': async (payload) => {
        const { runFullPipeline, setEventEmitter } = await import('./autonomous/pipeline.mjs');
        setEventEmitter((event, data) => this._emit(event, data));
        return runFullPipeline({
          minScore: payload.minScore ?? parseFloat(process.env.MIN_SCORE || '3.5'),
          maxApply: payload.maxApply ?? parseInt(process.env.MAX_APPLY_PER_RUN || '10'),
          workers: payload.workers ?? parseInt(process.env.PIPELINE_WORKERS || '3'),
          dryRun: payload.dryRun ?? process.env.SAFE_MODE !== 'false',
          autoTailor: true,
  ...
  async _processNext() {
    ...
    const job = getDb().transaction(() => {
      const row = getOne("SELECT id FROM job_queue WHERE state = 'queued' ORDER BY created_at ASC LIMIT 1");
      if (!row) return null;
      run("UPDATE job_queue SET state = 'running', started_at = datetime('now'), attempts = attempts + 1 WHERE id = ?", [row.id]);
      return getOne("SELECT * FROM job_queue WHERE id = ?", [row.id]);
    })();
    if (!job) return;

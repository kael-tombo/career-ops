 ...
 *   GEMINI_API_KEYS → comma-separated Gemini keys (fallback)
 *
...
async function tryCombinations(prompt, opts, providers, pIdx = 0, mIdx = 0) {
  if (pIdx >= providers.length) {
  ...
  try {
    if (prov.type === 'gemini') {
      return await callGeminiSDK(prov.resolvedKeys[0], model, prompt, opts);
    }
    return await callOpenAI(prov, model, prompt, opts);
  } catch (err) {
    const msg = err.message || '';
    const isRetryable = msg.includes('429') || msg.includes('401') || msg.includes('402')
    ...
    if (isRetryable) {
      ...
      if (nextModel < models.length) {
        return tryCombinations(prompt, opts, providers, pIdx, nextModel);
      }
      return tryCombinations(prompt, opts, providers, pIdx + 1, 0);
    }

    // For Gemini SDK 429 errors embedded in message
    if (msg.includes('[429]') || msg.includes('[429 Too Many Requests]')) {
      const nextModel = mIdx + 1;
      if (nextModel < models.length) {
        return tryCombinations(prompt, opts, providers, pIdx, nextModel);
      }
...
function getProviders() {
  if (_providerCache.length === 0) {
    _providerCache.push(...resolveProviders());
  }
...
export default { askGemini, askGeminiJSON, askGeminiArray };

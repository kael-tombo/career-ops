...
export function getProviderStatus() {
  const providers = [
    { id: 'openai', keyVar: 'OPENAI_API_KEY', models: 'gpt-4o, gpt-4o-mini, gpt-4-turbo' },
    ...
    { id: 'gemini', keyVar: 'GEMINI_API_KEY', models: 'gemini-2.0-flash, gemini-1.5-flash' },
  ];
  const order = (process.env.LLM_PROVIDER_ORDER || 'openai,grok,kimi,deepseek,gemini').split(',');
  return {
    order,
    providers: providers.map(p => ({
      ...p,
      configured: !!(process.env[p.keyVar] || process.env[`${p.keyVar}S`] || (p.id === 'gemini' && process.env.GEMINI_API_KEYS)),
    })),
  };
}
